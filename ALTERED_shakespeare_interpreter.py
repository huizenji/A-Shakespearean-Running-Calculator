    ...
    def _scene_number_from_roman_numeral(self, roman_numeral):
        for index, scene in enumerate(self.current_act.scenes):
            if scene.number == roman_numeral:
                return index
# ADDED    
    def _act_number_from_roman_numeral(self, roman_numeral):
        for index, act in enumerate(self.ast.acts):
            if act.number == roman_numeral:
                return index
# ADDED
    ...
    def _goto_scene(self, numeral):
        ...
        self.current_position['event'] = 0
# ADDED        
    def _goto_act(self, numeral):
    	act_number = self._act_number_from_roman_numeral(numeral)
    	self.current_position['act'] = act_number
    	self.current_position['event'] = 0
# ADDED
